(function() {
    //All the fly head parts with their colours
    var CLASS_INFO = [
...
function init(data) {
    ...
    fname.textContent = data.image_path.split('/').pop();

    //Fetches the raw TIFF file as an ArrayBuffer
    const url = '/' + data.image_path;
    fetch(url)
        .then(response => response.arrayBuffer())
        .then(buffer => {
            //Decode the TIFF using UTIF
            const ifds = UTIF.decode(buffer);
            UTIF.decodeImage(buffer, ifds[0]);
            const rgba = UTIF.toRGBA8(ifds[0]); //Gets raw RGBA pixels
            ...
            //Put the TIFF pixels onto the canvas
            const imgData = ctx.createImageData(imgW, imgH);
            imgData.data.set(rgba);
            ctx.putImageData(imgData, 0, 0);

            //Stores the image as a temporary canvas so draw() can still use it, and creates an offscreen canvas to act as img object
            const offscreen = document.createElement('canvas');
            offscreen.width = imgW;
            offscreen.height = imgH;
            offscreen.getContext('2d').putImageData(imgData, 0, 0);
            
            //Replace the global 'img' variable with our decoded canvas
            img = offscreen; 

            //Resume normal editor functions
            fitToWindow();
            buildList();
            draw();
        ...
        .catch(err => {
            ...
            showToast("Could not decode TIFF file", true);
            loader.style.display = 'none';
        });
}
    ...
    function buildList() {
        ...
        polys.forEach((p, i) => {
            console.log(i, p);
            console.log("points valid:", Array.isArray(p.points));
        });
        polylist.innerHTML = '';
        updateCount();
        for (var i = 0; i < polys.length; i++) {
            ...
            div.innerHTML =
                '<div class="ptop">' +
                    ...
                    '<span class="pconf">' + (((p.conf ?? 0) * 100).toFixed(0)) + '%</span>'
                '</div>' +
    ...
    btnfinish.addEventListener('click', function() {
        ...
        .then(function(data) {
            if (data.success) {
                ...
                setTimeout(function() {
                    window.location.href = data.redirect || '/folderR';
                }, 1200);
            } else {
